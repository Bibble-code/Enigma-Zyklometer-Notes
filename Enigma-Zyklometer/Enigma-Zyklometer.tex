...
Die Funktionen \textbf{Verschlüsselung} und \textbf{Zyklenerzeugung} erfordern die direkte Nutzung der nativen C-Implementierung. Diese wird über das Java Foreign Function and Memory API (FFM API) in die Spring Boot-Anwendung eingebunden. Der Datenfluss verläuft dabei vom Frontend über den Webserver und die REST-API bis zur C-Bibliothek und umgekehrt.

% Hier folgen die TikZ-Diagramme und der restliche Text unverändert
...
\begin{figure}[H]
	...
	\makebox[0pt][l]{%
		\begin{tikzpicture}[
			...
			box/.style={draw, rounded corners, minimum width=2.3cm, minimum height=1cm, align=center},
			arrow/.style={-Latex, thick}
			]
			\node[box] (clib) {C-Bibliothek};
			\node[box, right=of clib] (ffm) {Java FFM API};
			\node[box, right=of ffm] (api) {Java API};
			\node[box, right=of api] (nginx) {nginx};
			\node[box, right=of nginx] (frontend) {Frontend};
			
			\draw[<->, thick] (clib) -- (ffm);
			\draw[<->, thick] (ffm) -- (api);
			\draw[<->, thick] (api) -- (nginx);
			\draw[<->, thick] (nginx) -- (frontend);
		\end{tikzpicture}%
	}
	\caption{Datenfluss der Funktionen Verschlüsselung und Zyklenerzeugung}
	\label{fig:datenfluss-verschluesselung}
\end{figure}

Im Gegensatz dazu verläuft der Datenfluss bei der \textbf{Katalogabfrage} über eine PostgreSQL-Datenbank, die den Katalog der Charakteristiken enthält. Abfragen auf diesen Katalog werden innerhalb der Java-API über Hibernate und JDBC verarbeitet.
...
\begin{figure}[H]
	...
	\makebox[0pt][l]{%
		\begin{tikzpicture}[
			...
			\node[box] (db) {PostgreSQL};
			\node[box, right=of db] (jdbc) {JDBC + Hibernate};
			\node[box, right=of jdbc] (api2) {Java API};
			\node[box, right=of api2] (nginx2) {nginx};
			\node[box, right=of nginx2] (frontend2) {Frontend};
			
			\draw[<->, thick] (db) -- (jdbc);
			\draw[<->, thick] (jdbc) -- (api2);
			\draw[<->, thick] (api2) -- (nginx2);
			\draw[<->, thick] (nginx2) -- (frontend2);
		\end{tikzpicture}%
	}
	\caption{Datenfluss der Katalogabfrage}
	\label{fig:datenfluss-katalogabfrage}
\end{figure}
